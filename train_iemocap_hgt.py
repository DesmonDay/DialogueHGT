...
def train_or_eval_graph_model(model, loss_func, dataloader, epoch, cuda, optimizer=None, train=False):
    ...
    if preds != []:
        ...
        return float('nan'), float('nan'), [], [], float('nan'), []  # 验证集为空时，返回了 nan
...
if __name__ == "__main__":
    ...
    parser.add_argument('--graph-model', action='store_true', default=False,
                        help='whether to use graph model DialogueHGT after recurrent encoding')
    ...
    if args.graph_model:
        seed_everything(args.seed)
        model = DialogueHGTModel(args.base_model,
